...
function MainClass::HandleEvents()
{
  if(GSEventController.IsEventWaiting()) {
    ...
    if (ev == null) return;
    for (local ev_type = ev.GetEventType()){
          if(GSEvent.ET_COMPANY_NEW){
              local company_event = GSEventCompanyNew.Convert(ev);
              local company_id = company_event.GetCompanyID();
              GSGoal.Question(GSGoal.GoalID(GOAL_INVALID), company_id, Rules, GSGoal.QuestionType(QT_INFORMATION), GSGoal.QuestionButton(BUTTON_CONTINUE))
          }
      }
