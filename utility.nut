class CacheDuude {
	...
	constructor(strid, defvalue = 0) {
		this.id = strid;
		if (this.id in CacheDuude.indexer) {
			GSLog.Info(strid + " is already index");
			return;
		}
		this.index = CacheDuude.bigarray.len();
		for (local i = 0; i < 15; i++) {
			CacheDuude.bigarray.push(defvalue);
		}
...
function CacheDuude::Resolver() {
	...
	for (local r = 0; r < 15; r++) {
		...
		if (rc != GSCompany.COMPANY_INVALID) {
			comp.push(rc);
		}
	}
	return comp;
...
function CacheDuude::Vehicle_Helper_Counter(v_str) {
	...
	foreach(comp in company) {
		...
		if (v_str == "train" || v_str == "boat") {
			if (v_str == "train") {
				tl.KeepValue(GSVehicle.VT_RAIL);
			} else {
				tl.KeepValue(GSVehicle.VT_WATER);
			}
		} else {
			if (v_str == "bus" || v_str == "truck") {
				...
				local filter = GSList();
				foreach(veh, _ in tl) {
					filter.AddItem(veh, 0);
					foreach(pcargo, _ in pass) {
						if (GSVehicle.GetCapacity(veh, pcargo) > 0) {
							filter.SetValue(veh, 1);
						}
					}
				}
				if (v_str == "bus") {
				...
				foreach(veh, _ in tl) {
					...
					if (GSEngine.GetPlaneType(eng) == GSAirport.PT_HELICOPTER) {
						filter.SetValue(veh, 1);
					}
				}
				if (v_str == "aircraft") {
...
function CacheDuude::GetPassengerCargo() {
	...
	foreach(cargo, dummy in cargolist) {
		if (GSCargo.GetTownEffect(cargo) == GSCargo.TE_PASSENGERS) {
			CacheDuude.cargo.AddItem(cargo, 0);
		}
	}
	return CacheDuude.cargo;
}
...
function CacheDuude::GetSnowTown(addnew = false) {
	if (!CacheDuude.snowtown.IsEmpty() && !addnew) {
...
function CacheDuude::GetPowerPlant(addnew = false) {
	if (!CacheDuude.powerplant.IsEmpty() && !addnew && CacheDuude.powerplant.GetValue(0) != -1) {
		return CacheDuude.powerplant;
	}
	local cargolabel = ["COAL"];
	local cargo_list = GSCargoList();
	local energy_list = GSList();
	if (CacheDuude.powerplant.HasItem(0)) {
		CacheDuude.powerplant.RemoveItem(0);
	}
	...
	if (CacheDuude.powerplant.IsEmpty()) {
		CacheDuude.powerplant.AddItem(0, -1);
	}
	return CacheDuude.powerplant;
}
...
function CacheDuude::GetCompetitorStationAround(IndustryID) {
	...
	tiles.Valuate(GSTile.IsStationTile);
	tiles.KeepValue(1);
	tiles.Valuate(GSStation.GetStationID);
	foreach(tile, stationID in tiles) {
		if (!comp_list.HasItem(stationID)) {
			comp_list.AddItem(stationID, GSStation.GetOwner(stationID));
		}
	}
	return comp_list;
}
...
function CacheDuude::FindCargoUsage() {
	if (!CacheDuude.cargo_town.IsEmpty()) {
		return;
	}
		...
		if (sind < 1000000) {
			foreach(cargo, _ in CacheDuude.cargo_industry) {
				...
				foreach(industry, iii in ind_list) {
					...
					if (Utils.INArray(hh, CacheDuude.cargo_tracker) != -1)
					{
					...
					sind += z;
					if (sind > 1000000) {
						GSCargoMonitor.GetIndustryDeliveryAmount(company, cargo, industry, false);
					}
...
function CacheDuude::DeliveryAwardGeneric(number, comp) {
	if (Awards.HaveAward(number, comp)) {
		return;
	}
...
function CacheDuude::SetData(strID, companyID, data) {
	if (!(strID in CacheDuude.indexer)) {
		return -1;
	}
...
function CacheDuude::CalcCompanyValue(companyID) {
	...
	foreach(veh, price in vlist[companyID]) {
		value += price;
	}
	return value;
}
...
function CacheDuude::Company_Helper() {
	...
	for (local j = 0; j < 15; j++) {
		...
		if (c_idx != GSCompany.COMPANY_INVALID) {
			company.push(c_idx);
		}
	}
	return company;
