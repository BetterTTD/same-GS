...
class _SuperLib_Town
{
	 ...
	 * in desert or food in artic to grow, use GSTown.GetCargoGoal instead
...
function _SuperLib_Town::IsDesertTown(town_id)
{
	...
	if ("GetTerrainType" in GSTile) {
		return !GSTile.IsWaterTile(town_tile) && GSTile.GetTerrainType(town_tile) == GSTile.TERRAIN_DESERT;
	}
	...
	local result = _SuperLib_Town_Private_TownLandType(town_id, GSTile.IsDesertTile);
...
function _SuperLib_Town_Private_TownLandTypeScore(town_id, land_type_func, dir_list)
{
	...
	for (local i = 1; i < max_search; i++) {
		foreach (dir, _ in dir_list) {
			...
			if (GSTile.IsWaterTile(t)) {
				dir_status[dir] = false;
				n_unknown_dirs--;
			} else if (GSTile.IsBuildable(t) && !GSTile.HasTreeOnTile(t)) {
