...
class _SuperLib_Engine
{
	...
	/* vehicle_type = GSVehicle::VehicleType constant
	 */
...
function _SuperLib_Engine::GetRequiredStationType(engine)
{
	...
	switch(GSEngine.GetVehicleType(engine))
	{
		case GSVehicle.VT_ROAD:
			switch(GSRoad.GetRoadVehicleTypeForCargo(GSEngine.GetCargoType(engine)))
			{
				case GSRoad.ROADVEHTYPE_BUS:
					return GSStation.STATION_BUS_STOP;
				case GSRoad.ROADVEHTYPE_TRUCK:
	...
	_SuperLib_Log.Warning("Can't figure out which station type that corresponds to engine " + GSEngine.GetName(engine), _SuperLib_Log.LVL_INFO);
	return null;
}
...
{
	local engine_list = GSEngineList(vehicle_type);
	
	// Only keep engines that can cary the given cargo
	engine_list.Valuate(GSEngine.CanRefitCargo, cargo_id);
	engine_list.KeepValue(1);

	if(vehicle_type == GSVehicle.VT_ROAD)
	{
		if(no_trams)
		{
			// filter out trams
			engine_list.Valuate(GSEngine.GetRoadType);
			engine_list.KeepValue(GSRoad.ROADTYPE_ROAD);
		}
		if(no_articulated)
		{
			// filter out articulated vehicles
			engine_list.Valuate(GSEngine.IsArticulated)
			engine_list.KeepValue(0);
		}
	}

	if(vehicle_type == GSVehicle.VT_AIR && only_small_aircrafts)
	{
		engine_list.Valuate(GSEngine.GetPlaneType);
		engine_list.RemoveValue(GSAirport.PT_BIG_PLANE);
	}
...
function _SuperLib_Engine::GetFullSpeedTraveltime(engine, distance)
{
	...
	local km_h_speed = GSEngine.GetMaxSpeed(engine) * MULTI;
...
function _SuperLib_Engine::GetStationToStationAircraftTravelTime(engine, station1_id, station2_id)
{
	...
	local distance = GSMap.DistanceManhattan(ap_tiles[0], ap_tiles[1]);

	return _SuperLib_Engine.GetAircraftTravelTime(engine, distance, GSAirport.GetAirportType(ap_tiles[0]), GSAirport.GetAirportType(ap_tiles[1]));
}
...
function _SuperLib_Engine::GetAircraftTravelTime(engine, distance, station1_type, station2_type)
{
	...
	foreach(ap_type in ap_types)
	{
		switch(ap_type)
		{
			...
			case GSAirport.AT_SMALL:
				travel_time += 10;
				break;
			
			case GSAirport.AT_COMMUTER:
				travel_time += 5;
				break;

			case GSAirport.AT_LARGE:
				travel_time += 10;
				break;

			case GSAirport.AT_METROPOLITAN:
				travel_time += 8;
				break;

			case GSAirport.AT_INTERNATIONAL:
				travel_time += 7;
				break;

			case GSAirport.AT_INTERCON:
				travel_time += 7;
				break;
...
function _SuperLib_Private_GetBusEngineScoreValuator(engine_id, needed_capacity, max_money)
{
	...
	score += GSEngine.GetMaxSpeed(engine_id) * speed_factor;

	score += GSBase.RandRange(100);

	if(GSEngine.GetReliability(engine_id) > 50)
		score += GSEngine.GetReliability(engine_id) * 2;
...
function _SuperLib_Engine::GetEngine_PAXLink(needed_capacity, vehicle_type, only_small_aircrafts = false)
{
	...
	else if(vehicle_type == GSVehicle.VT_AIR)
	{
	...
	engine_list.Valuate(GSEngine.GetPrice);
	engine_list.RemoveValue(0);
	engine_list.Valuate(GSEngine.GetRunningCost);
	engine_list.RemoveValue(0);

	engine_list.Valuate(GSEngine.CanRefitCargo, _SuperLib_Helper.GetPAXCargo());
	engine_list.KeepValue(1);
	...
	if(_SuperLib_Log.IsLevelAccepted(_SuperLib_Log.LVL_DEBUG))
	{
		...
		{
			_SuperLib_Log.Info("Engine name: " + GSEngine.GetName(i), _SuperLib_Log.LVL_DEBUG);
		}
	}
	...
	engine_list.Sort(GSList.SORT_BY_VALUE, false); // highest first
...
/*static*/ function _SuperLib_Engine::GetIdealTransportDistance(engine_id, cargo_id, airport_type = -1)
{
			...
			abs(curr_distance - prev_distance) >= 5))   // stop if distance change was smaller than 5 tiles
	{
		...
		local value = GSCargo.GetCargoIncome(cargo_id, curr_distance, travel_time) * M / travel_time;
		local value_delta = GSCargo.GetCargoIncome(cargo_id, distance_delta, travel_time_delta) * M / travel_time;
