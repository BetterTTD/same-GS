...
// Return a nibble bitmask with each RAIL_BIT_NE,RAIL_BIT_SW,RAIL_BIT_NW,RAIL_BIT_SE direction set to 1
{
	...
	local trackMap = GSList();
	trackMap.AddItem(GSRail.RAILTRACK_NE_SW,	_SuperLib_Rail.RAIL_BIT_NE + _SuperLib_Rail.RAIL_BIT_SW);	// GSRail.RAILTRACK_NE_SW
	trackMap.AddItem(GSRail.RAILTRACK_NW_SE,	_SuperLib_Rail.RAIL_BIT_NW + _SuperLib_Rail.RAIL_BIT_SE);	// GSRail.RAILTRACK_NW_SE
	trackMap.AddItem(GSRail.RAILTRACK_NW_NE,	_SuperLib_Rail.RAIL_BIT_NW + _SuperLib_Rail.RAIL_BIT_NE);	// GSRail.RAILTRACK_NW_NE
	trackMap.AddItem(GSRail.RAILTRACK_SW_SE,	_SuperLib_Rail.RAIL_BIT_SW + _SuperLib_Rail.RAIL_BIT_SE);	// GSRail.RAILTRACK_SW_SE
	trackMap.AddItem(GSRail.RAILTRACK_NW_SW,	_SuperLib_Rail.RAIL_BIT_NW + _SuperLib_Rail.RAIL_BIT_SW);	// GSRail.RAILTRACK_NW_SW
	trackMap.AddItem(GSRail.RAILTRACK_NE_SE,	_SuperLib_Rail.RAIL_BIT_NE + _SuperLib_Rail.RAIL_BIT_SE);	// GSRail.RAILTRACK_NE_SE
	local railmask = 0;
...
function _SuperLib_Rail::AreRailTilesConnected(fromTile, toTile)
{
	...
	local own_company = GSCompany.ResolveCompanyID(GSCompany.COMPANY_SELF);
	if (GSTile.GetOwner(fromTile) != own_company) return false; // not own by us
	if (GSTile.GetOwner(toTile) != own_company)	  return false; // not own by us
	
	// Check that from and to tiles are the same rail type
	local from_rail_type = GSRail.GetRailType(fromTile);
	if (GSRail.GetRailType(toTile) != from_rail_type) return false; // not same railtype
	...
	local from_tile_mask = _SuperLib_Rail.GetRailBitMask(GSRail.GetRailTracks(fromTile));
	local to_tile_mask = _SuperLib_Rail.GetRailBitMask(GSRail.GetRailTracks(toTile));
	...
	if (GSRail.IsRailDepotTile(toTile)   && GSRail.GetRailDepotFrontTile(toTile) == fromTile)	to_tile_mask = to_tile_need;
	if (GSRail.IsRailDepotTile(fromTile) && GSRail.GetRailDepotFrontTile(fromTile) == toTile) from_tile_mask = from_tile_need;

	// Is toTile a bridge or tunnel?
	if (GSBridge.IsBridgeTile(toTile) || GSTunnel.IsTunnelTile(toTile))
	{
		local end_at = null;
		end_at = GSBridge.IsBridgeTile(toTile) ? GSBridge.GetOtherBridgeEnd(toTile) : GSTunnel.GetOtherTunnelEnd(toTile);
	...
	if (GSBridge.IsBridgeTile(fromTile) || GSTunnel.IsTunnelTile(fromTile))
	{
		local end_at = null;
		end_at = GSBridge.IsBridgeTile(fromTile) ? GSBridge.GetOtherBridgeEnd(fromTile) : GSTunnel.GetOtherTunnelEnd(fromTile);
