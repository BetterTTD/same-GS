...
class _SuperLib_Direction
{
	...
	/* Returns an GSList with all directions in random order */
	static function GetAllDirsInRandomOrder();

	/* Returns an GSList with all main directions in random order */
	static function GetMainDirsInRandomOrder();

	/* Returns an GSList with all diagonal directions in random order */
...
{
	local dir_list = GSList();
	for(local dir = _SuperLib_Direction.DIR_FIRST; dir != _SuperLib_Direction.DIR_LAST + 1; dir++)
	{
		dir_list.AddItem(dir, GSBase.Rand());
	}
	dir_list.Sort(GSList.SORT_BY_VALUE, GSList.SORT_DESCENDING);
...
{
	local rel_x = GSMap.GetTileX(adjacent_tile) - GSMap.GetTileX(tile);
	local rel_y = GSMap.GetTileY(adjacent_tile) - GSMap.GetTileY(tile);
...
{
	local rel_x = GSMap.GetTileX(tile2) - GSMap.GetTileX(tile1);
	local rel_y = GSMap.GetTileY(tile2) - GSMap.GetTileY(tile1);
