...
class _SuperLib_Helper
{
	 ...
	 */
	static function GetOpenTTDVersion(); // This method originates from GSLibCommon
	 ...
	 *
	 * If this method returns true, you should call GSController.Sleep(1) to
	...
	/* Gets the cargo in an GSList containing cargoes that has the highest
	...
	/**
	 * Apply a valuator function to every item of an GSList.
	 * @param list The GSList to apply the valuator to.
...
// This method originates from GSLibCommon
...
function _SuperLib_Helper::SetSign(tile, message, force_build_sign = false)
{
		...
		if(GSSign.GetLocation(i) == tile)
		{
			...
			else
			{
				if(message == "")
					...
					result = GSSign.SetName(i, message);
				found = true;
			}
		}
	}
	...
	if(!found)
		result = GSSign.BuildSign(tile, message);
...
function _SuperLib_Helper::ClearAllSigns(except_tile = -1)
{
	...
	foreach(i, _ in sign_list)
	{
		if (except_tile != -1 && GSSign.GetLocation(i) == except_tile) continue;
		GSSign.RemoveSign(i);
	}
}
...
{
	ai_list.Sort(GSList.SORT_BY_VALUE, true); // highest last
	return ai_list.GetValue(ai_list.Begin());
}

function _SuperLib_Helper::GetListMaxValue(ai_list)
{
	ai_list.Sort(GSList.SORT_BY_VALUE, false); // highest first
	return ai_list.GetValue(ai_list.Begin());
}
...
function _SuperLib_Helper::GetPAXCargo()
{
	if(!GSCargo.IsValidCargo(_SuperLib_Helper_private_pax_cargo))
	{
		...
		if(!GSCargo.IsValidCargo(pax_cargo))
		{
...
{
	if(!GSCargo.IsValidCargo(_SuperLib_Helper_private_mail_cargo))
	{
		local cargo_list = GSCargoList();
		cargo_list.Valuate(GSCargo.HasCargoClass, GSCargo.CC_MAIL);
		cargo_list.KeepValue(1);
		cargo_list.Valuate(GSCargo.GetTownEffect);
		cargo_list.KeepValue(GSCargo.TE_MAIL);

		local mail_cargo = _SuperLib_Helper.GetCargoWithLargestAvailabilityInLargestTown(cargo_list);

		if(!GSCargo.IsValidCargo(mail_cargo))
		{
...
function _SuperLib_Helper::GetCargoWithLargestAvailabilityInLargestTown(cargo_list)
{
	...
	if(GSTown.IsValidTown(top_town))
	{
		...
		cargo_list.Sort(GSList.SORT_BY_VALUE, GSList.SORT_DESCENDING);
