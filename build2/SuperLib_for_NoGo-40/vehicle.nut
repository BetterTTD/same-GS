...
class _SuperLib_Vehicle
{
	 ...
	 *
	 *     AddSetting({name = "use_rvs", description = "Enable road vehicles", easy_value = 1, medium_value = 1, hard_value = 1, custom_value = 1, flags = GSCONFIG_BOOLEAN | GSCONFIG_INGAME});
	 *     AddSetting({name = "use_planes", description = "Enable aircrafts", easy_value = 1, medium_value = 1, hard_value = 1, custom_value = 1, flags = GSCONFIG_BOOLEAN | GSCONFIG_INGAME});
	 *     AddSetting({name = "use_trains", description = "Enable trains", easy_value = 1, medium_value = 1, hard_value = 1, custom_value = 1, flags = GSCONFIG_BOOLEAN | GSCONFIG_INGAME});
	 *     AddSetting({name = "use_ships", description = "Enable ships", easy_value = 1, medium_value = 1, hard_value = 1, custom_value = 1, flags = GSCONFIG_BOOLEAN | GSCONFIG_INGAME});
	 *
	 *
	 ...
	 * at global scope:
	 *   GSOrder.GSOF_STOP_IN_DEPOT <- GSOrder.OF_STOP_IN_DEPOT;
	 *
	 *
	 * What it does:
...
/*static*/ function _SuperLib_Vehicle::GetVehicleLimit(vehicle_type)
{
	...
	if(GSGameSettings.IsDisabledVehicleType(vehicle_type))
		return 0;
	...
	return GSGameSettings.GetValue(setting);
}
...
/*static*/ function _SuperLib_Vehicle::IsVehicleTypeDisabledByAISettings(vehicle_type)
{
	...
	return GSController.GetSetting(ai_setting) == 0; // GetSetting returns -1 if the setting is undefined
}
...
{
	return GSVehicle.GetProfitLastYear(vehicle_id) + GSVehicle.GetProfitThisYear(vehicle_id);
}

/*static*/ function _SuperLib_Vehicle::IsVehicleAtStation(vehicle_id, station_id)
{
	if(GSVehicle.GetState(vehicle_id) != GSVehicle.VS_AT_STATION)
		return false;

	local veh_tile = GSVehicle.GetLocation(vehicle_id);

	// Check if the tile that the vehicle is at is a station
	// with the right station id
	return GSStation.GetStationID(veh_tile) == station_id;
}

/*static*/ function _SuperLib_Vehicle::HasTileListVehiclesOnIt(tile_rect, vehicle_type)
{
	local all_veh = GSVehicleList();
	all_veh.Valuate(GSVehicle.GetVehicleType);
...
{
	return tile_list.HasItem(GSVehicle.GetLocation(vehicle_id));
}

/*static*/ function _SuperLib_Vehicle::GetVehiclesAtTile(tile)
{
	local vehicles = GSVehicleList();
	vehicles.Valuate(GSVehicle.GetLocation);
...
/*static*/ function _SuperLib_Vehicle::GetVehicleLocations()
{
	...
	veh_list.Valuate(GSVehicle.GetLocation);
...
{
	local veh_list = GSVehicleList();
	veh_list.Valuate(GSVehicle.GetState);
	veh_list.KeepValue(GSVehicle.VS_CRASHED);
...
/*static*/ function _SuperLib_Vehicle::GetNonCrashedVehicleLocations(vehicle_type = null)
{
	local veh_list = GSVehicleList();
	veh_list.Valuate(GSVehicle.GetState);
	veh_list.RemoveValue(GSVehicle.VS_CRASHED);
	veh_list.RemoveValue(GSVehicle.VS_INVALID);
...
/*static*/ function _SuperLib_Vehicle::SendVehicleToDepotForSelling(vehicle_id, depot_tile, sell_status = "sell")
{
	...
	GSOrder.UnshareOrders(vehicle_id);
	while(GSOrder.GetOrderCount(vehicle_id) > 0)
	{
	...
	if(_SuperLib_Tile.IsDepotTile(depot_tile, GSVehicle.GetVehicleType(vehicle_id)))
	{
		// Send vehicle to specific depot so it don't get lost
		if(GSOrder.AppendOrder(vehicle_id, depot_tile, GSOrder.OF_STOP_IN_DEPOT))
		{
			...
			GSOrder.AppendOrder(vehicle_id, depot_tile, GSOrder.OF_STOP_IN_DEPOT);

			// so that vehicles that load stuff departures
			GSOrder.SkipToOrder(vehicle_id, 1); 
			GSOrder.SkipToOrder(vehicle_id, 0); 

			// Remove the second now unnecessary order
			GSOrder.RemoveOrder(vehicle_id, 1);
		}
	}
	else
	{
		// no depot at depot_tile => fallback to the API function
		GSVehicle.SendVehicleToDepot(vehicle_id);
	}

	// Turn around road vehicles that stand still, possible in queues.
	if(GSVehicle.GetVehicleType(vehicle_id) == GSVehicle.VT_ROAD)
	{
		if(GSVehicle.GetCurrentSpeed(vehicle_id) == 0)
		{
			...
			GSVehicle.ReverseVehicle(vehicle_id);
		}
	}
}
...
{
	local vehicle_list = GSVehicleList_Depot(depot_tile);
